    ...
    fn build(&self, app: &mut App) {
        app.init_resource::<DungeonGraph>()
            ...
            .add_systems(
                ...
                (
                    generate_dungeon,
                    build_inverted_echo_world,
                    ceiling::setup_ceiling,
                )
                    .chain(),
            )
...
pub(crate) struct DungeonGenParams<'w, 's> {
    pub commands: Commands<'w, 's>,
    pub config: Res<'w, GenerationConfig>,
    pub graph: ResMut<'w, DungeonGraph>,
    pub meshes: ResMut<'w, Assets<Mesh>>,
    pub floor_materials: ResMut<'w, Assets<crate::rendering::thermal::ThermalMaterial>>,
    pub water_materials: ResMut<'w, Assets<crate::rendering::WaterSurfaceMaterial>>,
    pub floor_wet_materials: ResMut<'w, Assets<crate::rendering::FloorWetMaterial>>,
    pub images: ResMut<'w, Assets<Image>>,
    pub asset_server: Res<'w, AssetServer>,
    pub reflection_tex: Res<'w, crate::rendering::ReflectionTexture>,
}

pub(crate) fn generate_dungeon(mut p: DungeonGenParams) {
    let unit_cube = p.meshes.add(Cuboid::new(1.0, 1.0, 1.0));
    let _unit_plane = p.meshes.add(Plane3d::default());

    let map_w = (176.0 * p.config.scale) as i32;
    let state = BspState {
        width: map_w,
        depth: map_w,
    ...
    let avg_scaled = p.config.average_room_size * p.config.scale;

    let (mut rooms, edges, build_room_geometry) = match p.config.layout_mode.as_str() {
        "arena" => {
            let margin = 8.0_f32;
        ...
        "snake3d" => {
            let (rooms, edges) =
                state.generate_snake3d(p.config.snake_cell_size, p.config.snake_layers);
            (rooms, edges, true)
        }
        "maze3d" => {
            let (rooms, edges) = state.generate_maze3d(
                p.config.maze_cell_size,
                p.config.maze_layers,
                p.config.maze_loop_chance,
                p.config.maze_vertical_link_chance,
            );
            (rooms, edges, true)
        }
        "labyrinth" => {
    ...
    p.graph.edges = edges.clone();
    p.graph.rooms = rooms.clone();
    println!(
        "Dungeon generated: {} rooms, {} edges",
        rooms.len(),
        edges.len()
    );
    ...
    let _checker_tex = p.images.add(checker_img);

    // Python parity: floor color (0.19, 0.22, 0.28), wall color (0.78, 0.82, 0.88)
    let room_tex: Handle<Image> = p.asset_server.load("graphics/rooms/00110-1042801366.png");
    let mat_floor_wet = p
        .floor_wet_materials
        .add(crate::rendering::FloorWetMaterial {
    ...
    let mat_water = p
        .water_materials
        .add(crate::rendering::WaterSurfaceMaterial {
            base: StandardMaterial {
            ...
            extension: crate::rendering::WaterSurfaceExtension {
                ...
                reflection_texture: Some(p.reflection_tex.0.clone()),
            },
        });

    let mat_corridor_floor = p
        .floor_materials
        .add(crate::rendering::thermal::ThermalMaterial {
    ...
    if build_room_geometry {
        for r in rooms.iter() {
            ...
            let collision_groups = bevy_rapier3d::prelude::CollisionGroups::new(
                layer_group,
                bevy_rapier3d::prelude::Group::all(),
            );

            p.commands.spawn((r.clone(), SpatialBundle::default()));
            ...
            p.commands.spawn((
                ...
                MaterialMeshBundle {
                    ...
                    transform: Transform::from_xyz(cx, cz - p.config.floor_thickness * 0.5, cy)
                        .with_scale(Vec3::new(r.w, p.config.floor_thickness, r.h)),
                    ..default()
                },
                RigidBody::Fixed,
            ...
            let wet_t = (p.config.floor_thickness * 0.12).max(0.01);
            p.commands.spawn((
                Spatial4D {
                    w: r.w_layer as f32 * 5.0,
                    target_w: r.w_layer as f32 * 5.0,
            ...
            let visual_wall_h = p.config.room_height;
            let wall_t = p.config.wall_thickness;
            let wall_y = cz + visual_wall_h * 0.5;
            let door_half = p.config.corridor_width * 0.5 + wall_t * 0.5;
            ...
            let mut spawn_wall_segment = |pos: Vec3, size: Vec3| {
                ...
                p.commands.spawn((
                    MaterialMeshBundle {
                        mesh: p.meshes.add(Cuboid::new(size.x, size.y, size.z)),
            ...
            let base_open = {
                ...
                let target = p.config.corridor_width + wall_t * 1.1;
            ...
            let lane_half = (door_w * 0.56).max(p.config.corridor_width * 0.58) + 0.42;
            let inset = 1.05_f32.max(p.config.corridor_width * 0.42) + 0.42;
            ...
            for &px in &pxs {
                for &pz in &pzs {
                    ...
                    p.commands.spawn((
                        MaterialMeshBundle {
                            mesh: p.meshes.add(Cuboid::new(0.3, pillar_h, 0.3)),
                            material: mat_corridor_wall.clone(),
            ...
            if r.w >= 14.0 && r.h >= 14.0 && rng.gen_bool(0.12) {
                ...
                let thickness = p.config.wall_thickness * 0.76;
                let wz = cz + p.config.room_height * 0.5;
                ...
                for &(acx, acy, angle) in corners.iter().take(use_count) {
                    ...
                    angled_boxes.push((
                        pos,
                        rot,
                        Vec3::new(half_len, p.config.room_height, thickness),
                    ));
                }
                ...
                if !angled_boxes.is_empty() {
                    ...
                    p.commands.spawn((
                        MaterialMeshBundle {
                            mesh: p.meshes.add(create_merged_box_mesh_rotated(&angled_boxes)),
                            material: mat_angled,
    ...
    if build_room_geometry {
        ...
        for &(a, b) in &edges {
            ...
            let wall_collision_groups = CollisionGroups::new(wall_group, wall_group);
            let cz = 0.0;
            let spatial = Spatial4D {
                w: ra.w_layer as f32 * 5.0,
                target_w: ra.w_layer as f32 * 5.0,
                layer: ra.w_layer,
                is_folded: false,
            };
            ...
            if seg1_len > 0.6 {
                ...
                let sy = ca.y;
                corridor_floor_boxes.push((
                    Vec3::new(sx, cz - p.config.floor_thickness * 0.5, sy),
                    Vec3::new(
                        seg1_len,
                        p.config.floor_thickness,
                        p.config.corridor_width + p.config.wall_thickness,
                    ),
                ));
                for &side in &[1.0_f32, -1.0] {
                    corridor_wall_boxes.push((
                        Vec3::new(
                            sx,
                            cz + visual_wall_h * 0.5,
                            sy + side
                                * (p.config.corridor_width * 0.5 + p.config.wall_thickness * 0.5),
                        ),
                        Vec3::new(
                            seg1_len + p.config.wall_thickness,
                            visual_wall_h,
                            p.config.wall_thickness,
                        ),
                    ));
                }
            }
                ...
                for &side in &[1.0_f32, -1.0] {
                    corridor_wall_boxes.push((
                        ...
                        Vec3::new(
                            p.config.wall_thickness,
                            visual_wall_h,
                            seg2_len + p.config.wall_thickness,
                        ),
                    ));
                }
            }
            ...
            // Joint
            let joint_size = p.config.corridor_width + p.config.wall_thickness;
            corridor_floor_boxes.push((
                Vec3::new(mid_x, cz - p.config.floor_thickness * 0.5, mid_y),
                Vec3::new(joint_size, p.config.floor_thickness, joint_size),
            ));
            ...
            for sx in [-1.0, 1.0] {
                for sy in [-1.0, 1.0] {
                    corridor_wall_boxes.push((
                        Vec3::new(mid_x + sx * ox, cz + visual_wall_h * 0.5, mid_y + sy * oy),
                        Vec3::new(
                            p.config.wall_thickness,
                            visual_wall_h,
                            p.config.wall_thickness,
            ...
            if !corridor_floor_boxes.is_empty() {
                let floor_mesh = p.meshes.add(create_merged_box_mesh(&corridor_floor_boxes));
                let floor_parts: Vec<(Vec3, Quat, Collider)> = corridor_floor_boxes
                    .iter()
                    .map(|(p, s)| {
                        (
                            *p,
                            Quat::IDENTITY,
                            Collider::cuboid(s.x * 0.5, s.y * 0.5, s.z * 0.5),
                        )
                    })
                    .collect();
                p.commands.spawn((
                ...
                p.commands.spawn((
                    MaterialMeshBundle {
                        mesh: p.meshes.add(create_merged_box_mesh(&wet_boxes)),
                        material: mat_floor_wet.clone(),
                        transform: Transform::IDENTITY,
                        ..default()
                    },
    ...
    if p.config.layout_mode == "arena" {
        for room in &p.graph.rooms {
    ...
    p.commands.spawn((
        MaterialMeshBundle {
            mesh: p.meshes.add(Plane3d::new(
                Vec3::Y,
                Vec2::new(boundary_half_x, boundary_half_z),
            )),
    ...
    p.commands.spawn((
        ...
        CollisionGroups::new(
            Group::from_bits_truncate(0x7FFFFFFF),
            Group::from_bits_truncate(0x7FFFFFFF),
        ),
        MapGeometry,
    ));

    let (high_y, low_y) = if p.config.layout_mode == "arena" {
        (15.3, -15.3)
    } else {
        (p.config.room_height + 4.0, -1.4)
    };
...
fn build_inverted_echo_world(
    ...
    water_sources: EchoSourceQuery<crate::rendering::WaterSurfaceMaterial>,
    thermal_sources: EchoSourceQuery<crate::rendering::thermal::ThermalMaterial>,
    floor_wet_sources: EchoSourceQuery<crate::rendering::FloorWetMaterial>,
) {
