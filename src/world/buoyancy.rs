...
fn apply_water_buoyancy(
    ...
    let Ok((mut ext_force, mut vel, mass_props, player_tf, _spatial)) = query.get_single_mut()
    else {
        return;
    };

    let ball_pos = player_tf.translation;
    let ball_radius: f32 = 0.68;
    ...
    let mass = match mass_props {
        ...
        ColliderMassProperties::Density(d) => {
            *d * (4.0 / 3.0) * std::f32::consts::PI * ball_radius.powi(3)
        }
        ColliderMassProperties::MassProperties(_) => 1.25,
    };
    ...
    for (water_tf, surface) in water_query.iter() {
        ...
        if dx <= surface.half_extents.x && dz <= surface.half_extents.y {
            ...
            if max_water_h.is_none_or(|prev_h| h > prev_h) {
                max_water_h = Some(h);
            }
        }
    }
