...
type ItemPickupQueryResult = (
    Entity,
    &'static Transform,
    Option<&'static ExpOrb>,
    Option<&'static HealthPowerup>,
    Option<&'static SwordPowerup>,
);

type WaterCrystalQueryResult = (
    Entity,
    &'static mut Transform,
    &'static mut WaterCrystal,
    &'static mut Handle<StandardMaterial>,
);

type ItemBobQueryResult = (
    &'static mut Transform,
    Option<&'static ExpOrb>,
    Option<&'static HealthPowerup>,
    Option<&'static SwordPowerup>,
...
struct ItemPickupParams<'w, 's> {
    ...
    player_query: Query<
        ...
        With<Player>,
    >,
    item_query: Query<'w, 's, ItemPickupQueryResult, With<Item>>,
    xp_events: EventWriter<'w, crate::systems::progression::GainXpEvent>,
    heal_events: EventWriter<'w, crate::systems::progression::HealEvent>,
    sword_events: EventWriter<'w, crate::systems::progression::SwordPowerupEvent>,
    fx_events: EventWriter<'w, crate::effects::FloatingTextEvent>,
    sfx_events: EventWriter<'w, crate::effects::audio::PlaySfxEvent>,
}

#[derive(bevy::ecs::system::SystemParam)]
struct WaterCrystalParams<'w, 's> {
    time: Res<'w, Time>,
    commands: Commands<'w, 's>,
    player_query: Query<'w, 's, &'static Transform, With<Player>>,
    crystal_query: Query<'w, 's, WaterCrystalQueryResult, (With<WaterCrystal>, Without<Player>)>,
    materials: ResMut<'w, Assets<StandardMaterial>>,
    meshes: ResMut<'w, Assets<Mesh>>,
    rapier_context: Res<'w, bevy_rapier3d::prelude::RapierContext>,
}
...
fn spawn_monsters_items_handler(
    ...
    for room in graph.rooms.iter() {
        if health_spawned >= health_count {
            break;
        }
    ...
    for event in events.read() {
        match event {
            SpawnItemEvent::Exp { pos, amount } => {
                ...
                while remaining > 0.0 {
                    let val = (remaining * rng.gen_range(0.35..0.7))
                        .max(1.0)
                        .min(remaining)
                        .clamp(1.0, 3.2);
                    remaining -= val;
...
fn item_bobbing(time: Res<Time>, mut query: Query<ItemBobQueryResult, With<Item>>) {
...
fn item_attraction(
    ...
    mut item_query: Query<ItemAttrQueryResult, (With<Item>, Without<Player>)>,
) {
...
fn item_pickup_collision(mut params: ItemPickupParams) {
    if let Ok((player_tf, _progression)) = params.player_query.get_single() {
        let p_pos = player_tf.translation;
        let p_radius = 0.6; // Ball radius approx

        for (entity, i_tf, exp, health, sword) in params.item_query.iter_mut() {
            let dist = p_pos.distance(i_tf.translation);
            ...
            if dist < (p_radius + pickup_radius) {
                params.sfx_events.send(crate::effects::audio::PlaySfxEvent {
                    ...
                    volume: 0.7,
                    pitch: 1.0 + rand::random::<f32>() * 0.2,
                    position: Some(i_tf.translation),
                });
                if let Some(e) = exp {
                    params
                        .xp_events
                        .send(crate::systems::progression::GainXpEvent { amount: e.value });
                    ...
                    params
                        .heal_events
                        .send(crate::systems::progression::HealEvent { amount: h.heal });
                    params.fx_events.send(crate::effects::FloatingTextEvent {
                        pos: i_tf.translation + Vec3::Y * 0.4,
                    ...
                    params
                        .sword_events
                        .send(crate::systems::progression::SwordPowerupEvent {
                            powerup_type: s.powerup_type.clone(),
                        });
                    ...
                    let (color, text) = match s.powerup_type.as_str() {
                        ...
                        _ => (
                            Color::srgba(1.0, 0.88, 0.38, 1.0),
                            s.powerup_type.to_uppercase(),
                        ),
                    };
...
fn update_water_crystals(mut params: WaterCrystalParams, mut spawn_timer: Local<f32>) {
    let dt = params.time.delta_seconds();
    let roll_time = params.time.elapsed_seconds();

    // 1. Spawning
    *spawn_timer -= dt;
    if let Ok(player_tf) = params.player_query.get_single() {
        let mut rng = rand::thread_rng();
        // Python parity: interval = 0.3s
        if *spawn_timer <= 0.0 {
            let count = params.crystal_query.iter().count();
            if count < 180 {
                *spawn_timer += 0.3; // Reset timer
                ...
                params.commands.spawn((
                    PbrBundle {
                        mesh: params.meshes.add(Cuboid::new(0.4, 0.4, 1.0)),
    ...
    for (entity, mut tf, mut crystal, mat_handle) in params.crystal_query.iter_mut() {
        ...
        match crystal.state {
            ...
            CrystalState::Fading => {
                ...
                if let Some(mat) = params.materials.get_mut(mat_handle.id()) {
