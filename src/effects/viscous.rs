...
impl Plugin for ViscousDistortPlugin {
    fn build(&self, app: &mut App) {
        ...
        app.init_resource::<ViscousUpdateTimer>();

        app.add_systems(
            Update,
            update_viscous_distortion
        ...
        render_app
            .add_render_graph_node::<ViewNodeRunner<ViscousDistortNode>>(
                Core3d,
                ViscousDistortLabel,
            )
...
impl ViewNode for ViscousDistortNode {
    type ViewQuery = (&'static ViewTarget, &'static ViscousSettings);
...
impl FromWorld for ViscousDistortPipeline {
    fn from_world(world: &mut World) -> Self {
        ...
        let shader = world
            ...
            .load("shaders/viscous_distort.wgsl");
...
fn update_viscous_distortion(
    ...
    player_query: Query<
        (
            ...
            &crate::components::CompressionState,
        ),
        With<crate::player::Player>,
    >,
    mut timer: ResMut<ViscousUpdateTimer>,
    time: Res<Time>,
) {
    let dt = time.delta_seconds();
    timer.accumulator += dt;
    let interval = 1.0_f32 / 30.0_f32;
    if timer.accumulator < interval {
        return;
    }
    timer.accumulator -= interval;
    ...
    let compress = (1.0_f32 - factor).max(0.0_f32);
    let dilate = (factor - 1.0_f32).max(0.0_f32);
    let compression_intensity = (compress / 0.92_f32).clamp(0.0_f32, 1.0_f32);
    let dilation_intensity = (dilate / 0.9_f32).clamp(0.0_f32, 1.0_f32);
    let timespace_intensity = compression_intensity.max(dilation_intensity * 0.42_f32);
