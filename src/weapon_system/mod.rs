...
type MonsterQuery<'w, 's, F> =
    Query<'w, 's, (Entity, &'static Transform, &'static Spatial4D), (With<crate::ai::Monster>, F)>;
...
fn spawn_blade_echoes(
    ...
    for (mut weapon, tf) in query.iter_mut() {
        if !matches!(weapon.state, WeaponState::Spin) {
            continue;
        }
...
fn update_hyperbomb(
    ...
    monster_query: MonsterQuery<Without<Hyperbomb>>,
    time: Res<Time>,
...
fn update_magic_missiles(
    ...
    mut missile_query: Query<
        (Entity, &mut MagicMissile, &mut Transform, &Spatial4D),
        Without<crate::ai::Monster>,
    >,
    monster_query: MonsterQuery<Without<MagicMissile>>,
    time: Res<Time>,
    mut damage_events: EventWriter<DamageEvent>,
) {
    let dt = time.delta_seconds();
    for (entity, mut missile, mut tf, sp) in missile_query.iter_mut() {
        ...
        if let Some(target_entity) = missile.target {
            if let Ok((_, target_tf, _)) = monster_query.get(target_entity) {
                ...
                missile.velocity =
                    missile.velocity + (target_dir * 25.0 - missile.velocity) * (dt * 7.6);
            } else {
                missile.target = None;
            }
        }
