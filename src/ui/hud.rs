        ...
        root.spawn((
            ...
            parent.spawn(NodeBundle {
                ...
                mouse.spawn(NodeBundle {
                    ...
                    spawn_mouse_button(buttons, "L", MouseButton::Left, &asset_server, Val::Percent(30.0), true);
                    spawn_mouse_button(buttons, "M", MouseButton::Middle, &asset_server, Val::Percent(30.0), true);
                    spawn_mouse_button(buttons, "R", MouseButton::Right, &asset_server, Val::Percent(30.0), true);
                });
            ...
            top_left.spawn(NodeBundle {
                ...
                bar_bg.spawn((
                    NodeBundle {
                        ...
                        background_color: BackgroundColor(Color::srgba(0.32, 0.95, 1.0, 0.24)),
                ...
                bar_bg.spawn((
                    NodeBundle {
                        ...
                        background_color: BackgroundColor(Color::srgba(0.24, 0.95, 0.54, 0.96)),
                        ..default()
                    },
                    HpBarFill,
                ));
            ...
            top_left.spawn(NodeBundle {
                ...
                bar_bg.spawn((
                    NodeBundle {
                        ...
                        background_color: BackgroundColor(Color::srgba(0.25, 0.9, 1.0, 0.18)),
                        ..default()
                    },
                    XpBarGlow,
                ));
                ...
                bar_bg.spawn((
                    NodeBundle {
                        ...
                        background_color: BackgroundColor(Color::srgba(0.68, 0.18, 0.12, 0.92)),
                        ..default()
                    },
                    XpBarFill,
                ));
...
pub(crate) fn update_ui(
    time: Res<Time>,
    player_query: Query<(&PlayerStats, &PlayerProgression)>,
    mut hp_bar_query: Query<(&mut Style, &mut BackgroundColor), (With<HpBarFill>, Without<XpBarFill>, Without<HpBarGlow>, Without<XpBarGlow>)>,
    mut xp_bar_query: Query<(&mut Style, &mut BackgroundColor), (With<XpBarFill>, Without<HpBarFill>, Without<XpBarGlow>, Without<HpBarGlow>)>,
    mut hp_glow_query: Query<(&mut Transform, &mut BackgroundColor), (With<HpBarGlow>, Without<HpBarFill>, Without<XpBarFill>, Without<XpBarGlow>)>,
    mut xp_glow_query: Query<(&mut Transform, &mut BackgroundColor), (With<XpBarGlow>, Without<XpBarFill>, Without<HpBarFill>, Without<HpBarGlow>)>,
    mut level_text_query: Query<&mut Text, (With<LevelHudText>, Without<HoloMapTitle>, Without<HpLabelText>)>,
    mut hp_label_query: Query<&mut Text, (With<HpLabelText>, Without<LevelHudText>, Without<HoloMapTitle>)>,
    mut title_query: Query<&mut Text, (With<HoloMapTitle>, Without<LevelHudText>, Without<HpLabelText>)>,
) {
    ...
    if let Ok((stats, level)) = player_query.get_single() {
        if let Ok((mut style, mut bg)) = hp_bar_query.get_single_mut() {
        ...
        if let Ok((mut transform, mut bg)) = hp_glow_query.get_single_mut() {
        ...
        if let Ok((mut style, mut bg)) = xp_bar_query.get_single_mut() {
        ...
        if let Ok((mut transform, mut bg)) = xp_glow_query.get_single_mut() {
        ...
        if let Ok(mut text) = hp_label_query.get_single_mut() {
...
pub(crate) fn update_monster_hud_text(
    ...
    if let Ok(mut text) = text_query.get_single_mut() {
        text.sections[0].value = format!("Slayed: {}\nLeft: {}", mob_stats.slain, mob_stats.total.saturating_sub(mob_stats.slain));
    }
}
...
pub(crate) fn update_input_hud(
    ...
    mut key_nodes: Query<(&HudKeyNode, &mut BackgroundColor, &mut BorderColor, &Children), Without<HudMouseNode>>,
    mut mouse_nodes: Query<(&HudMouseNode, &mut BackgroundColor, &mut BorderColor, &Children), Without<HudKeyNode>>,
    ...
    for (node, mut bg, mut border, children) in key_nodes.iter_mut() {
        ...
        for &child in children.iter() {
            if let Ok(mut text) = text_query.get_mut(child) {
                text.sections[0].style.color = if pressed { Color::WHITE } else { Color::srgba(0.9, 0.94, 1.0, 0.92) };
            }
        }
    }
