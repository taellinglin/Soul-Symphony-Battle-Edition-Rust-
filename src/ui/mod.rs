...
use crate::systems::progression::{GameState, PlayerProgression};
use bevy::prelude::*;
        ...
        app.init_resource::<GameOverCountdown>()
            ...
            .add_systems(
                ...
                (
                    ...
                    update_client_list_ui,
                    update_network_debug_ui,
                    update_game_over_countdown,
                    hud::update_monster_hud_text,
                    hud::update_boss_room_ui,
                    hud::update_input_hud,
                ),
            );
    }
}
...
fn update_client_list_ui(
    ...
    if let Ok((stats, level)) = player_query.get_single() {
        if let Ok(mut text) = text_query.get_single_mut() {
            ...
            lines.push(format!(
                "{}  Lv {}  HP {:.0}/{:.0}  XP {:.0}/{:.0}",
                local_name, level.level, stats.hp, stats.max_hp, level.xp, level.xp_next
            ));
